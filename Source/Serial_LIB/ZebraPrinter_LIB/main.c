/***************************************************************************//*!
* \file main.c
* \author Zedagim Dalfa
* \copyright Arxtron Technologies Inc. All Rights Reserved.
* \date 10/28/2020 10:00:29 AM
							 ...
							 void *callbackData, int eventData1, int eventData2)
{
	switch (event)
	{
		case EVENT_COMMIT:
			...
			*/
			if(!strcmp(fnName, "Initialize_ZebraPrinter_Lib"))
				DebugLibFn(Initialize_ZebraPrinter_Lib, inputTypes, parseArg, errmsg);
...
int main (int argc, char *argv[])
{
	...
	*/
	AddFnToDropdown(Initialize_ZebraPrinter_Lib, 0x11, PrinterSerialName, errmsg);
	...
	char configFile[MAX_PATHNAME_LEN] = {0};
	sprintf(configFile, "%s\\Serial.xml", currentDir);
	
	fprintf (stderr, "Initializing Serial Library\n");
	tsErrChk (InitializeSerialPortLib(configFile, glbMainPanelHandle, errmsg), errmsg);
	fprintf (stderr, "Serial Library Initialized\n");
	
	fprintf (stderr, "Initializing Library\n");
	tsErrChk (Initialize_ZebraPrinter_Lib("Printer Serial Name", errmsg), errmsg);
